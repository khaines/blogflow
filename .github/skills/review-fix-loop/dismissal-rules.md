...
- If `consensus = "1/3"` AND `severity` is Medium → dismiss with **explicit rationale required** (brief investigation before dismissing).
- If `consensus = "1/3"` AND `severity` is High → **DO NOT auto-dismiss**. Investigate the finding. If confirmed as false positive or design choice, dismiss under those categories (§3 or §4) with documented rationale. Otherwise, treat as actionable.
...
**Action**: Categorize as "Dismissed — Low consensus" with a note about which model flagged it and the council coverage (e.g., "1/3 — Sonnet timed out").
...
- **Security** — authentication, authorization, credential handling, injection risks
- **Content integrity** — path traversal protection, content isolation between sites, unauthorized content access, cross-site content leakage
- **Data integrity** — schema correctness, referential integrity, data loss risks, missing constraints
- **Cryptography** — key management, token validation, signature verification

**Rationale**: These domains have outsized blast radius. A single missed content-integrity gap or security vulnerability can cause production incidents. The cost of investigating a false positive in these domains is far lower than the cost of dismissing a real issue.
...
- Check out the base branch and verify whether the same issue exists there.
...
- If the PR *copied* a problematic pattern to a new location, the NEW instance IS in scope (the PR introduced it).
...
**Rationale**: Some findings are technically valid observations but conflict with intentional architectural or design choices. If the choice is documented (in an ADR, PR description, or code comment), the finding is not actionable in this context.

**How to detect**:

- Check whether the finding conflicts with an Architecture Decision Record (ADR) in `docs/engineering/adr/`.
...
**Rationale**: Style preferences vary between models. If the code follows the project's coding conventions (checklist 03, 03a), a model's alternative style suggestion is not actionable.
...
2. **Protected domain safety net** — If the finding touches a protected domain (§2.1: security, content integrity, data integrity, cryptography), skip low-consensus auto-dismissal. The finding must be investigated and dismissed only under §3 (pre-existing), §4 (design choice), or §5 (style) with explicit rationale.
...
6. **Low consensus** — Does it meet the consensus threshold for the current council coverage? (See §2 for full/degraded/severely-degraded thresholds.)
...
2. **Fixing it is impossible in this PR** — not merely difficult or time-consuming, but genuinely requires:
   - Changes to a different component or repository
   - An upstream dependency change or version bump
   - An ADR decision that hasn't been made yet
   - Infrastructure that doesn't exist yet (e.g., "add monitoring dashboard" when no monitoring stack exists)
3. **The finding does not affect security, content integrity, or data integrity** — Critical findings in these domains are NEVER deferred. They block the PR.

### When deferral is NOT acceptable

- "It would take too long" — fix it or increase max_rounds.
- "It's an edge case" — edge cases cause production incidents. Fix it.
- "We'll get to it later" — later never comes. Fix it now.
- "It's only Medium severity" — Medium findings that are in-diff and fixable must be fixed.
- "The code works without it" — working code is not the bar. Quality code is the bar.

### Deferral reporting

Every deferred finding MUST include:
- A specific explanation of WHY it cannot be fixed in this PR
- A **GitHub issue number** tracking the follow-up — this is mandatory, not optional:
  - If an existing issue already covers the deferred work, reference it by number
  - If no existing issue covers it, **create a new issue immediately** before finalizing the deferral
  - The issue must include: the deferred finding description, why it was deferred, the risk assessment, and a reference back to the PR that deferred it
  - Vague references to epics or ADRs are NOT sufficient — there must be a specific, actionable issue that will appear in the backlog
...
After all dismissals are finalized in a round, run a **triage verification** on dismissed findings to catch false dismissals. This step is mandatory when **5 or more findings** are dismissed in a single round, and recommended for any round with dismissed findings in protected domains (§2.1).
...
dismissed as "low consensus (1/4)" but we need to verify it isn't a
...
Empirical data shows that a significant percentage of 1/4-consensus dismissed findings can be real issues when re-evaluated by other models. The highest-impact false dismissals tend to be in security and data-integrity domains. The triage step catches these at minimal cost (a few minutes of model time) compared to the risk of shipping real issues.
